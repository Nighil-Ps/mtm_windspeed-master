...
try:
        ...
        conn = engine.connect()
        adadata = metadata.tables['ADA_DATA_MTM_chk_valid']
...
def validation():
	...
	for ves_id in ves_lst:
		utc_param = conn.execute(select([adadata.c.ID,adadata.c.UTC_REPORT_DATE_TIME,adadata.c.VESSEL_id]).where(and_(adadata.c.VESSEL_id==str(ves_id),adadata.c.validation == None ,adadata.c.outlier == None,adadata.c.UTC_REPORT_DATE_TIME > '2020-05-30 00:00:00' )).order_by(asc(adadata.c.UTC_REPORT_DATE_TIME))).fetchall()
		...
		while(start<=max_time[0]):
			...
			heading_diff_lst=[]
			outlier_lst=[]
			valid_param = conn.execute(select( [adadata.c.SOG,adadata.c.STW,adadata.c.WIND_SPEED,adadata.c.TORQUE,adadata.c.POWER,adadata.c.RPM,adadata.c.DRAFT_MEAN,adadata.c.RUDDER_ANGLE,adadata.c.WIND_DIRECTION,adadata.c.HEADING,adadata.c.ID,adadata.c.UTC_REPORT_DATE_TIME,adadata.c.DRAFT_FORE,adadata.c.DRAFT_AFT,adadata.c.VESSEL_id]).where(and_(adadata.c.UTC_REPORT_DATE_TIME.between(start,end),adadata.c.validation==None,adadata.c.outlier==None,adadata.c.VESSEL_id==str(ves_id),adadata.c.UTC_REPORT_DATE_TIME > '2020-05-30 00:00:00')).order_by(asc(adadata.c.UTC_REPORT_DATE_TIME))).fetchall() 
			...
			if not valid_param:
				...
				if(len(sog_lst)!=0 and len(stw_lst)!=0 and len(wind_speed_lst)!=0 and len(torque_lst)!=0 and len(power_lst)!=0 and len(rpm_lst)!=0 and len(draft_mean_lst)!=0):
					...
					if(sog_std_error>0.5 or stw_std_error >0.5 or rpm_std_error>3):
						x=1
