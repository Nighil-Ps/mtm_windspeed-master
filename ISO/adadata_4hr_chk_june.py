...
def ada_4hr():
	...
	for ves_id in ves_lst:
		...
		utc_param = conn.execute(select([adadata.c.ID,adadata.c.UTC_REPORT_DATE_TIME,adadata.c.check_status]).where(and_(adadata.c.check_status==None,adadata.c.VESSEL_id==str(ves_id),adadata.c.UTC_REPORT_DATE_TIME > '2020-05-30 00:00:00')).order_by(asc(adadata.c.UTC_REPORT_DATE_TIME))).fetchall()
