...
def calc_wind_speed():
	# q1="SELECT UID FROM NOONDATA2 WHERE DATE(mail_date) = DATE(NOW())"
	q1="SELECT UID FROM NOONDATA2 WHERE Vessel_Name='MTH' "
	# q1="SELECT UID FROM NOONDATA2 WHERE WindSpeed_kn IS NULL"
	...
	for vessel_uid in vessels:
		...
		q3="SELECT WindForce FROM NOONDATA2 WHERE UID=\'"+str(vessel_uid[0])+"\'"
		...
		if(mean_wind):
			q5="UPDATE NOONDATA2 SET WindSpeed_kn=\'"+str(mean_wind[0][0])+"\' WHERE UID=\'"+str(vessel_uid[0])+"\'"
			res=executeQ(q5)
		else:
			print("no windforce")
def status_alert(e):
	...
	body = "MTM NOONDATA2 WindSpeed_kn calculation not completed due to the error :-\n\n"+str(e)
